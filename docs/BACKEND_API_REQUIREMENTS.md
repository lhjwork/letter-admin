...
실물 편지 상태 표시 시스템을 단순화하여 편지 중심의 관리 시스템으로 개선합니다. 또한 수신자 정보(이름, 휴대폰 번호, 주소, 메시지)를 쉽게 확인할 수 있는 기능을 추가합니다.
...
- 간단하고 직관적인 UI
- 편지별 통합 상태 관리
- **수신자 정보 (이름, 휴대폰 번호, 주소, 메시지) 쉽게 확인 가능**
...
1. **실물 편지 신청 목록 페이지** (`/physical-letters/requests`)

   - 모든 실물 편지 신청을 테이블 형태로 표시
   - 수신자 이름, 휴대폰 번호, 배송 주소, 메모 정보 포함
   - 편지 정보와 상태도 함께 표시

2. **편지별 관리 페이지 개선** (`/letters/physical`)

   - "상세보기" 버튼 추가로 각 편지의 실물 편지 신청 상세 정보 확인 가능
   - 모달 창에서 해당 편지의 모든 실물 편지 신청 목록과 수신자 정보 표시

3. **네비게이션 개선**
   - 실물 편지 관리 메인 페이지에서 "신청 목록 보기" 버튼으로 쉽게 접근
   - 브레드크럼 네비게이션으로 페이지 간 이동 편의성 향상

### 기술적 구현

- `PhysicalLetterRequests.tsx`: 신청 목록 전용 페이지
- `PhysicalLetterDetailsModal.tsx`: 편지별 상세 정보 모달
- 기존 API (`/admin/physical-requests`) 활용하여 수신자 정보 표시
- 반응형 디자인으로 모바일에서도 정보 확인 가능
